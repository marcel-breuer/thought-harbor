...
test('walks the source, review, knowledge, and grounded chat journey', async ({ page }) => {
  ...
  const uploadedSource = page.getByRole('button', { name: /notes\.txt transcript · 24 B/ });
  await expect(uploadedSource).toBeVisible();
  await expect(uploadedSource.getByText('ready', { exact: true })).toBeVisible();
  ...
  await page.getByRole('textbox', { name: 'Question' }).fill('Where does the decision belong?');
