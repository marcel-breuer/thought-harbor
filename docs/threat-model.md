...
| Internet-exposed | Credential attacks, CSRF, upload abuse, proxy misconfiguration | HTTPS, strong secrets, proxy rate limits, patched images, backups, no direct database/Redis exposure |

PostgreSQL, Redis, and MCP are internal Compose services by default. API,
worker, and MCP containers make outbound HTTPS requests to OpenRouter. Only
the web service is published. Operators who expose the API or MCP server must
put them behind authenticated TLS and a network-level rate limiter.
...
OpenRouter is required for chat, extraction, and embeddings. Prompts and
selected source content leave the installation and are handled by the provider
behind the selected model. The Profile and Settings surfaces disclose this.
Operators should review OpenRouter's and model providers' data retention terms.
HTML email bodies are not rendered by the first parser; only text/plain
content is indexed. Do not treat a private self-hosted deployment as safe
against a compromised host, malicious administrators, or compromised
